...
\title{Binarize}
\author{Tamara J. Bl\"atte, Stefan Mundus, Ludwig Lausser, Martin Hopfensitz, \\ Christoph M\"ussel and Hans A. Kestler}
...
As already mentioned, all binarization routines in the package provide a statistical testing procedure that evaluates the reliability of the binarization. While the dip test employed with $k$-means is based on the distribution of the original data, the bootstrap test of BASC assesses the variability of potential binarization thresholds.
