...
dbl_array* e_tri = 0;
int_array* e_returned_tri = 0;

double calc_jump_height_tri(int_matrix* P, dbl_array* vect, int i, int k, int j){
 ...
 */
double calc_error_tri(int_matrix* P, dbl_array* vect, int i, int k, int j){

	double z1, z2, err, e_root, tmp;
	int d, index;

	//check if value was computed before
	index = P->values[j][i]-1;
	if(e_tri && e_returned_tri && e_returned_tri->values[index])
	{
		e_returned_tri->values[index]++;
		return e_tri->values[index];
	}
	...
	if(e_tri && e_returned_tri)
	{
		e_returned_tri->values[index]++;
		e_tri->values[index] = err;
	}
 ...
 */
double calc_score_tri(int_matrix* P, dbl_matrix* H1, dbl_matrix* H2, dbl_array* vect, int i, int k, int j){

	double score,score1,score2;

	score1 = calc_jump_height_tri(P, vect, i, -1, j);
	score2 = calc_jump_height_tri(P, vect, -1, k, j);
	H1->values[j][i] = score1;
	H2->values[j][k] = score2;
	score = score1+score2;
	score /= 2*calc_error_tri(P, vect, i, k, j);
 ...
 */
void calc_V_tri(int_matrix* v, dbl_array* Q_max, dbl_matrix* Q, dbl_matrix* H1, dbl_matrix* H2, int_matrix* P, dbl_array* vect){
	...
	for(j = 1; j < P->rows; j++){
		...
		for(i = 0; i <= (j-1); i++){
			...
			for(k = (i+1); k <= j; k++){
				...
				cur_val = calc_score_tri(P, H1, H2, vect, i, k, j);
 ...
 */
void calc_final_results_tri(final_result_tri* result, int_matrix* v, dbl_array* vect, dbl_array* vect_sorted, double tau, int numberofsamples)
{
	int i, k, c;
...
//	print_int_matrix(v, "v");

//	print_int_array(v_col1, "v_col1");
//	print_int_array(v_col2, "v_col2");
...
//		print_int_array(samples, "samples");
 ...
 */
int alloc_Accelerator_Memory_tri(int value_count){

	int bytes = 0;

	e_tri = init_dbl_array(0, value_count - 1, 0);
	bytes += value_count * sizeof(double);
	e_returned_tri = init_int_array(0, value_count - 1, 0);
...
SEXP TASCA(SEXP vect, SEXP tau, SEXP numberofsamples){
	...
	alloc_Accelerator_Memory_tri(value_count);
	...
	calc_V_tri(v_vec, Q_Max_vec, Q_Mat, H_Mat1, H_Mat2, P_Mat, vect_sorted);
	
	//free the memory for calculated values
	free_Accelerator_Memory();
	free_Accelerator_Memory_tri();

	//calculate the final three results
	calc_final_results_tri(&f_res, v_vec, vector, vect_sorted, *REAL(tau), *INTEGER(numberofsamples));
...
void calc_V_Scalespace_tri(calc_V_result_tri* result, mgs_result* mgs_res, quant_result* q_res, dbl_matrix* H1, dbl_matrix* H2, dbl_array* vect){
	dbl_array *smoothed,*step_heights,*step_heights2;
	double *smoothedSlopes, mn1, mn2, max_quot, cur_quot;
...
*/
		for(j = 0; j < q_res->steps->cols && q_res->steps->values[i][j]; j++){
			...
			for(k = (j+1); k < q_res->steps->cols && q_res->steps->values[i][k]; k++){
				...
				cur_quot /= 2*(cost_Scalespace(smoothed, 0, (j-1), mn1)
					+ cost_Scalespace(smoothed, k, smoothed->length - 1, mn2)
					+ 0.5*(cost_Scalespace(smoothed, j, k - 1, mn1)
						+ cost_Scalespace(smoothed, j, k - 1, mn2)));
...
SEXP TASCB(SEXP vect, SEXP tau, SEXP numberofSamples, SEXP sigma){
	...
	calc_V_Scalespace_tri(&v_res, &mgs_res, &q_res, H_Mat1, H_Mat2, vect_sorted);
	...
	calc_final_results_tri(&f_res, v_res.v, vector, vect_sorted, *REAL(tau), *INTEGER(numberofSamples));
